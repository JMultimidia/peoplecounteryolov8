...
#        print(row)
    ...
    for bbox in bbox_id:
        ...
        #print(results)
        if results>=0:
           people_entering[id]=(x4,y4) #store the id and the position of the person entering the area
           cv2.rectangle(frame, (x3, y3), (x4, y4), (0, 0, 255), 2)

        if id in people_entering: #check if the id is in the people_entering dictionary
           results1 = cv2.pointPolygonTest(np.array(area1, np.int32), (x4, y4), False)
           if results1>=0: #check if the person is in the area1
        ...
        # print(results)
        if results2 >= 0:
            people_exiting[id] = (x4, y4)  # store the id and the position of the person exiting the area
            cv2.rectangle(frame, (x3, y3), (x4, y4), (0, 255, 0), 2)

        if id in people_exiting:  # check if the id is in the people_exiting dictionary
            results3 = cv2.pointPolygonTest(np.array(area2, np.int32), (x4, y4), False)
            if results3 >= 0:  # check if the person is in the area1
    ...
    cv2.putText(frame, "ENTRARAM: "+str(i), (20, 20), cv2.FONT_HERSHEY_SIMPLEX, (0.5), (0, 255, 0), 2)
    cv2.putText(frame, "SAIRAM: "+str(o), (20, 40), cv2.FONT_HERSHEY_SIMPLEX, (0.5), (0, 0, 255), 2)
